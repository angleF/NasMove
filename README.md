...
- 支持 NAS IP、主机名和 `smb://` 输入；成功连接后自动恢复最近一次连接配置，密码只从 macOS Keychain 读取。
- 使用 SMB 3.x、签名和可选加密访问群晖共享；连接测试覆盖 TCP、SMB 协商、认证和共享访问。
- 支持多个本地文件／目录，能够追加、移除选中项或清空列表，固定保留目录层级并保留空目录。
- NAS 目标目录只能通过远端目录浏览器选择，共享根层不能作为目标；同名冲突自动保留扩展名并重命名。
...
- 启动首页为任务工作台，左侧任务列表、右侧状态详情；通过“新建任务”进入连接、来源与目标选择。
- 复制进度随已落盘检查点更新，校验进度随远端分块读取更新；重连显示实际重试次数和等待时间。
- 执行异常显示中文原因与处理建议，安全错误码保存到 SQLite；可查看分页文件结果、导出摘要、在 Finder 中查看目标目录。
- SQLite 为主线程、规划线程和执行线程分别创建连接，避免跨线程连接错误；所有连接保持完整同步与外键约束。
