...
已重新生成可启动的 `dist/NasMove.app`，bundle ID 固定为 `com.nasmove.app`，依赖锁哈希、凭据扫描、ad-hoc 完整性检查和启动烟测通过。当前机器没有 Developer ID Application 证书，Python／Qt 工具链为 `x86_64`，且新的本地持久身份仍等待首次钥匙串私钥授权，所以现有包仅用于本机开发验证，不满足正式外部分发条件。
...
| pytest（功能与包检查分开执行） | 通过：功能 957 passed，12 skipped；包检查 2 passed | 两次退出码均为 0 |
| Ruff | 通过 | 退出码 0 |
| mypy strict | 通过：55 个源码文件 | 退出码 0 |
...
| 本机代码签名完整性 | 部分通过：ad-hoc 完整性有效；持久身份等待首次钥匙串授权 | 本地验收须完成持久身份签名；正式发布必须使用 Developer ID Application |
...
最近一次验证日期：2026-09-11。新应用已重新编译，包检查 `2 passed`，ad-hoc 完整性和启动烟测通过；持久身份最终签名仍等待钥匙串授权，详见 [任务工作台验证记录](task-workbench-verification.md)。此前真实 NAS 故障回归为 `36 passed, 1 skipped`；该 skip 是需要显式授权的 DSM 重启。本轮未重复执行真实 NAS 门禁，历史结果见 Synology 验证矩阵。

## 人工验收

- 首次启动时数据目录权限为 `0700`，SQLite、锁和日志能够正常创建。
- 输入 IP、主机名和 `smb://` 地址分别完成连接测试。
- 保存密码后退出并升级应用，相同连接 UUID 能从本机数据库读取凭据。升级自使用 macOS Keychain 的旧版本时，旧密码不会迁移，需要重新输入一次（无凭据时任务按既有协议暂停）。
- 选择多个文件和目录，确认层级、Unicode 文件名和空目录保持正确。
- 复制任务中执行暂停、恢复、断网和强制退出；检查 `.part`、检查点和最终 SHA-256。
- 移动任务只有在目标完整校验后将源文件移入废纸篓；模拟入篓失败时目标与源均保留并显示警告。
...
- 新版出现数据或传输故障时停止分发，不删除 SQLite 状态、已保存凭据或 NAS 上的 `.part`。
